...
inline const union keyseq_u *get_mapped_key(enum keylayout_e mode, uint8_t controller, enum row_e row, uint8_t col, enum neo_levels_e level) {
    return &(keymap[mode][row][controller][col].seq[level]);
}

inline const struct keyleveltranslations_s *get_mapped_klt(enum keylayout_e mode, uint8_t controller, enum row_e row, uint8_t col) {
