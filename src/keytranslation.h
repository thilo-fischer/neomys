...
/**
 * @file Defines those symbols used to specify the key presses (or
 * sequences of key presses) to be sent to the USB host as reaction to
 * the key presses done on the Neomys keyboard.
 *
 * Key presses on the Neomys keyboard can be translated to key presses
 * (or sequences of key presses) that will make the USB host the
 * Neomys is connected to (e.g. a PC) act as if it uses the Neo2
 * keyboard layout even if it uses a different layout, e.g. DE , US
 * [TODO] or US-international [TODO]. The Neomys software running on
 * the keyboard will take care of these translations, the USB host
 * does not need any special modifications or software. This file
 * defines the symbols used to specify the necessary translation
 * tables in keymappings.c.
 */
